...
class AdvanceRestApi(http.Controller):
	def __init__(self):
		self._token = request.env["api.access.token"]
	...
	#select model
	@validate_token
	@http.route("/api/<model>", "/api/<model>/<id>", "/api/<model>/<id>/<action>",
	...
	def get_lock_date(self,**kw):
		access_token = request.httprequest.headers.get("access_token")
		access_token_data = (request.env["api.access.token"].sudo().search([("token", "=", access_token)], order="id DESC", limit=1))
