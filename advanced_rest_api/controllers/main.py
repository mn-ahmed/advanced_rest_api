...
class Dn_api(http.Controller):
	...
	def api_get(self, model=None, id=None, **payload):
		...
		if model:
			...
			return {'status': 404, 'state': 'Failed',
	...
	def post_data(self, model=None, id=None, **payload):
		...
		if model:
			...
			respond = {'status': 404, 'state': 'Failed',
					   'message': "The model %s is not available in the registry." % ioc_name}
		...
		if not _model:
			...
			if not payload['id']:
				...
				try:
					...
					respond = {'status': 200, 'state': 'Success',
							   'message': "update %s record %s successfully!" % (_model.model, payload['id'])}
		return respond
		...
		if not _model:
			...
			try:
				...
				if record:
					...
					respond = {'status': 401, 'state': 'Failed',
							   'message': 'record object with id %s could not be found' % payload['id']}
			except Exception as e:
				request.env.cr.rollback()
				respond = {'status': 401, 'state': 'Failed', 'message': e}
			else:
				...
				respond = {'status': 200, 'state': 'Success',
						   'message': "delete %s record %s successfully!" % (_model.model, payload['id'])}
		return respond
